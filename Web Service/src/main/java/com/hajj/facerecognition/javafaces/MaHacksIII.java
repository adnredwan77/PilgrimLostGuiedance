package com.hajj.facerecognition.javafaces;
...
// The output can be assigned either to a BufferedImage or to an Image
    ...
    public static float comparePeeps(Image compare, Image current) {
        ...
        faceCascade.load("C:\\Users\\Abdelrahman\\Documents\\workspace-sts-3.9.0.RELEASE\\OpenCVTest\\src\\facerecognition\\javafaces\\haarcascade_frontalface_alt.xml");
...
//                        System.out.println((facesArray[i].br().x-facesArray[i].tl().x)*(facesArray[i].tl().y-facesArray[i].br().y)+"  "+camera.get(CV_CAP_PROP_FRAME_WIDTH)*camera.get(CV_CAP_PROP_FRAME_HEIGHT)/5);
                        if (-1 * (facesArray[i].br().x - facesArray[i].tl().x) * (facesArray[i].tl().y - facesArray[i].br().y) > 
                        camera.get(CV_CAP_PROP_FRAME_WIDTH) * camera.get(CV_CAP_PROP_FRAME_HEIGHT) / 8) {
                        //if(true) {
                        	if (!faceDetect) {
                                faceDetect = true;
                                face2 = true;
...
//                                    TestFaceRecognition.main(arguments);
                                	//FaceRecognition.parseDirectory(System.getProperty("user.home") + "\\SecureWebCam", "jpg").size()
                                    result = model.processSelections(dir.getPath(), System.getProperty("user.home") + "\\SecureWebCam",
                                    		4, 5 + "");
...
//                                }
                                ...
                                if (result != null && result.getMatchSuccess()) {
                                    System.out.println("Match!  "+result.getMatchFileName());
